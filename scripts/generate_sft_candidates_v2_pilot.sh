...
# V2 step-SFT on-policy rejected candidate pilot.
# Selects 10 samples per category (60 total), generates two candidates on
# each GPU, and merges them into four candidates per sample.

SOURCE="data/logickor_sft_high_v2.jsonl"
RUN_DIR="data/logickor_sft_candidate_v2_pilot"
TASKS="${RUN_DIR}/pilot_tasks.jsonl"
...
python augmentation/generate_sft_candidates_vllm.py prepare \
  --source "${SOURCE}" \
  --output "${TASKS}" \
  --pilot-per-category "${PILOT_PER_CATEGORY}" \
