...
logickor_eval/    LogicKor 원본 기반 생성, Judge 평가, 점수 집계 코드
augmentation/     preference 후보 생성, 판정, 데이터 확정 코드
...
평가에는 병합 모델 또는 `adapter_config.json`과 어댑터 가중치가 있는 LoRA adapter를 사용할 수 있음
...
### 검수 완료 preference 데이터로 DPO

Codex 검수를 통과한 데이터는 한 줄에 하나의 JSON 객체로 저장합니다. `prompt`는 정확히
`user → assistant → user` 세 메시지이며, `chosen`과 `rejected`는 마지막 assistant 답변 문자열입니다.

```json
{"id": 1, "category": "추론", "prompt": [{"role": "user", "content": "..."}, {"role": "assistant", "content": "..."}, {"role": "user", "content": "..."}], "chosen": "...", "rejected": "..."}
```

데이터를 `data/logickor_dpo_reviewed.jsonl`에 둔 뒤 먼저 모델을 로드하지 않는 dry-run으로
스키마, train/eval 분리, 토큰 길이와 잘림 여부를 확인합니다.

```bash
python train/train_dpo.py \
  --config configs/train_gemma4_e4b_dpo.yaml \
  --output-dir /tmp/logickor-dpo-dry-run \
  --dry-run
```
...
DPO policy와 reference는 모두 기존 `runs/gemma4_e4b_step_sft/adapter`에서 시작합니다.
같은 base 모델에 SFT adapter를 policy와 고정 reference로 각각 로드하므로 모델 본체를 GPU에 복제하거나 병합하지 않습니다. 학습 결과는 병합하지 않고 `runs/gemma4_e4b_dpo_full/adapter`에 저장됩니다.
...
LoRA adapter를 미리 병합하지 않고 실행하려면 adapter 디렉토리를 모델 인자로 전달함

```bash
python logickor_eval/generator.py \
  --model runs/gemma4_e4b_dpo_full/adapter \
  --gpu_devices 0 \
  --model_len 4096
```

`generator.py`는 adapter의 `adapter_config.json`에서 base model과 LoRA rank를 읽고, vLLM에서 LoRA를 실행 시점에 적용함. 전체 모델을 병합하거나 별도 저장하지 않으며, 결과는 병합 모델을 사용한 경우와 같은 `base + LoRA` 가중치로 생성됨. base model과 adapter를 명시적으로 분리하려면 `--model google/gemma-4-E4B-it --adapter runs/gemma4_e4b_dpo_full/adapter` 형식도 사용할 수 있음.

`generator.py`는 `logickor_eval/questions.jsonl`을 읽고, 현재 작업 디렉토리 기준 `generated/<model-or-adapter-path>/` 아래에 결과를 저장함
...
2. merged model은 디스크를 크게 사용함. LogicKor 생성은 LoRA adapter를 vLLM에서 직접 적용할 수 있으므로 평가만을 위해 미리 병합할 필요는 없음
